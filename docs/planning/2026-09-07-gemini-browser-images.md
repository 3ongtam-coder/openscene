...
3. Generate starts a hidden real Chromium renderer at `labs.google/fx/tools/flow`, opens the newest project or creates one, switches the prompt configuration to Image, chooses x1 and Landscape/Portrait, and maps the catalog selection to Nano Banana 2 or Nano Banana Pro. If the account does not expose the requested model, the job fails instead of silently using another one.
