...
- `product-chrome` is a compact top bar with the current page label, the project pill, a concise `Local` indicator, and icon-only Back, Projects, Home (Menu), and Settings buttons. Back walks a bounded in-app page-history stack (workspace pages included, workspace panel restored on return), stays disabled when there is no history, and applies the same project-required guard as forward navigation. It reads as the desk toolbar, not the product billboard; the Edit workspace keeps its own branding visually hidden for accessibility only. The Menu button stays disabled with a short hint until a project is open; Projects and Settings stay reachable at all times.
...
- The Edit Agent can see project footage through the read-only `watchProjectVideo` MCP tool (converted from the claude-video /watch skill): the main process plans duration-budgeted uniform frame timestamps (denser on a focused `startMs`–`endMs` range, 2 fps rate cap, ≤20 frames), extracts width-512 JPEGs with the discovered FFmpeg runtime from the asset's confined project folder, and the agent graph attaches them as a multimodal image message. Frame base64 never enters the tool transcript, chat history, or renderer — the tool/user messages carry only a timestamped summary, and extraction errors are path-free.
...
- Settings owns theme preferences, provider connections, endpoints, primary model configuration, and local AI engine preferences. It is not part of `APP_WORKSPACES` and must not appear in `AppWorkspaceNavigation`.
- Model access follows the opencode provider → model concept via the shared `llmProviders.ts` registry. Settings → Providers mirrors opencode's layout: a **Connected providers** list (hairline-separated rows with a source tag — `Local` for Ollama with its endpoint field inline, `API key` for cloud — and a ghost Disconnect) above a **Popular providers** list of unconnected rows with an "Unlocks …" note and a `+ Connect` button that opens a modal connect dialog (`Connect {Provider}`, write-only password field into main-process safe storage, required-key validation, Cancel/Connect). Settings → Models mirrors opencode's model manager: a search field over the catalog grouped by provider, each row with a `role="switch"` visibility toggle persisted locally; hidden models disappear from the Edit Agent picker and the primary model select while the active selection always stays listed. The Edit Agent prompt bar uses `AgentModelPicker`: models grouped by provider with per-provider connection state, cloud models disabled until their provider is connected, and the selection persisted per domain. Chat sending gates on "selected model's provider is connected", not "local only" — OpenAI, Anthropic, Google Gemini, and DeepSeek have real main-process adapters for both completions and Edit Agent tool calling, and API keys never enter the renderer.
