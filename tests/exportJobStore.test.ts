...
describe('export job store', () => {
  it('tracks queued, running progress, and completed states without storing an output path', () => {
    ...
    const completed = store.markCompleted(queued.id, 'export_01.mp4', 1234, undefined, 'export_01.srt');

    expect(completed.state).toMatchObject({ kind: 'completed', fileName: 'export_01.mp4', fileSizeBytes: 1234, subtitleFileName: 'export_01.srt' });
