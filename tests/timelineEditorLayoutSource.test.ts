...
describe('timeline editor layout source contract', () => {
  ...
  it('Given product identity, When the app shell and editor render, Then Window Loom branding belongs to the program header', async () => {
    const source = await readTimelineEditorSource();
    const appShellSource = await readAppShellSource();

    expect(source).toContain('<p className="section-kicker">Local studio</p>');
    expect(source).toContain('<h1 id="timeline-editor-title">Window Loom</h1>');
    ...
    expect(appShellSource).not.toContain('<h1 id="app-title">Window Loom</h1>');
